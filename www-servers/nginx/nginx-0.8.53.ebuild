...
# $Header: /var/cvsroot/gentoo-x86/www-servers/nginx/nginx-0.8.53.ebuild,v 1.1 2010/11/28 21:21:35 hollow Exp $
...
src_install() {
	...
	if use nginx_modules_http_passenger; then
		...
		doruby lib/phusion_passenger.rb

		insinto /usr/share/phusion-passenger/source/ext/ruby
		doins ext/ruby/*

		exeinto /usr/bin
		doexe bin/passenger-memory-stats bin/passenger-status
		doexe bin/passenger-config bin/passenger-stress-test
		doexe bin/passenger-make-enterprisey
		...
		doexe agents/PassengerLoggingAgent agents/PassengerWatchdog
